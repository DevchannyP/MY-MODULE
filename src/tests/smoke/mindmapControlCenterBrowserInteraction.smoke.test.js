...
test('[mindmap control center browser interaction smoke] selected worker wiring reaches send-now runtime path', async (t) => {
  ...
  try {
    ...
    assert.match(mindmap.text, /function stageRunCurrentState\(report\)/);
    assert.match(mindmap.text, /function stageRunRetryableSummary\(report\)/);
    assert.match(mindmap.text, /function stageRunEvidenceStatus\(report\)/);
    assert.match(mindmap.text, /function stageRunEvidenceError\(report\)/);
    assert.match(mindmap.text, /function stageRunRequestId\(report\)/);
    assert.match(mindmap.text, /function stageRunCorrelationId\(report\)/);
    assert.match(mindmap.text, /function stageRunTraceQuery\(report\)/);
    assert.match(mindmap.text, /function copyStageRunTraceQuery\(historyIndex\)/);
    assert.match(mindmap.text, /function loadStageRunTracePrompt\(historyIndex\)/);
    assert.match(mindmap.text, /function sendStageRunTracePrompt\(historyIndex\)/);
    ...
    assert.match(mindmap.text, /현재 상태/);
    assert.match(mindmap.text, /실패 위치/);
    assert.match(mindmap.text, /실패 원인/);
    assert.match(mindmap.text, /운영 증거/);
    assert.match(mindmap.text, /저장 오류/);
    assert.match(mindmap.text, /요청 ID/);
    assert.match(mindmap.text, /상관 ID/);
    assert.match(mindmap.text, /로그 확인/);
    assert.match(mindmap.text, /즉시 전송 가드/);
    assert.match(mindmap.text, /가능한 다음 행동/);
    assert.match(mindmap.text, /재실행/);
    assert.match(mindmap.text, /첫 실패 불러오기/);
    assert.match(mindmap.text, /실행 패널에 채우기/);
    assert.match(mindmap.text, /즉시 전송/);
    assert.match(mindmap.text, /로그 추적 복사/);
    ...
    assert.match(mindmap.text, /operator_guidance/);
    assert.match(mindmap.text, /runtime_observability/);
    assert.match(mindmap.text, /확인 필요 \/ 저장 관측 정보 없음/);
    assert.match(mindmap.text, /요청 ID: /);
    assert.match(mindmap.text, /상관 ID: /);
    assert.match(mindmap.text, /request_id=/);
    assert.match(mindmap.text, /correlation_id=/);
    assert.match(mindmap.text, /requested_stage=/);
    assert.match(mindmap.text, /requested_module=/);
    assert.match(mindmap.text, /대기 \/ 연결된 PTY 세션이 없어 즉시 전송할 수 없습니다\./);
    assert.match(mindmap.text, /가능 \/ 현재 선택된 PTY 세션으로 즉시 전송할 수 있습니다\./);
    assert.match(mindmap.text, /showToast\('stage 실행 추적값 실행 패널 반영 완료'\);/);
    assert.match(mindmap.text, /sendPromptNow\(\);/);
    assert.match(mindmap.text, /onclick="sendStageRunTracePrompt\(' \+ failureFocusEntry\.index \+ '\)"' \+ htmlDisabled\(traceSendDisabled\)/);
    assert.match(mindmap.text, /onclick="sendStageRunTracePrompt\(' \+ entry\.index \+ '\)"' \+ htmlDisabled\(traceSendDisabled\)/);
    assert.match(mindmap.text, /onclick="sendStageRunTracePrompt\(\)"' \+ htmlDisabled\(traceSendDisabled\)/);
    ...
    assert.match(mindmap.text, /window\.copyStageRunTraceQuery = copyStageRunTraceQuery;/);
    assert.match(mindmap.text, /window\.loadStageRunTracePrompt = loadStageRunTracePrompt;/);
    assert.match(mindmap.text, /window\.sendStageRunTracePrompt = sendStageRunTracePrompt;/);
    assert.match(mindmap.text, /Operator Chain/);
    assert.match(mindmap.text, /Release Evidence/);
    assert.match(mindmap.text, /Action Sources/);
    assert.match(mindmap.text, /Home Action/);
    assert.match(mindmap.text, /home-spotlight/);
    assert.match(mindmap.text, /control-center/);
    assert.match(mindmap.text, /function latestOperatorActionForScope\(scope\)/);
    assert.match(mindmap.text, /function operatorActionDeliverySummary\(entry\)/);
    assert.match(mindmap.text, /function operatorActionSourceSummary\(entries\)/);
    assert.match(mindmap.text, /function normalizeOperatorActionClientEntries\(entries\)/);
    assert.match(mindmap.text, /function loadOperatorActionClientStorage\(storageKey\)/);
    assert.match(mindmap.text, /function mergeOperatorActionClientEntries\(entries, action\)/);
    assert.match(mindmap.text, /function operatorActionClientSourceSummary\(entries\)/);
    assert.match(mindmap.text, /function normalizeDeepLinkClientContext\(context\)/);
    assert.match(mindmap.text, /function loadDeepLinkClientStorage\(storageKey\)/);
    assert.match(mindmap.text, /function readDeepLinkClientContextFromSearch\(searchValue\)/);
    assert.match(mindmap.text, /function buildDeepLinkClientHref\(basePath, focus, targetId, meta\)/);
    assert.match(mindmap.text, /function fetchJsonClient\(url, options\)/);
    assert.match(mindmap.text, /function copyTextToClipboardClient\(value\)/);
    assert.match(mindmap.text, /function reserveClientIdempotencyKey\(scope, payload\)/);
    assert.match(mindmap.text, /최근 전달/);
    assert.match(mindmap.text, /function renderOperatorChainCards\(\)/);
    assert.match(mindmap.text, /function executionOperatorCockpitChainSummary\(\)/);
    assert.match(mindmap.text, /scope.indexOf\('chain:'\) === 0/);
