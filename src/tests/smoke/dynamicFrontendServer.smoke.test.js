...
test('[dynamic frontend server smoke] node server renders dynamic frontend surfaces and keeps planning api alive', async (t) => {
  ...
  try {
    ...
    assert.match(home.text, /문제 해결 제어 센터/);
    assert.match(home.text, /같은 자동화 요청을 다시 저장해도 중복으로 처리되지 않도록 안전장치가 들어 있습니다\./);
    ...
    assert.match(home.text, /아직 기록 없음/);
    assert.match(home.text, /문제 해결 화면 열기/);
    assert.match(home.text, /검사 보드 열기/);
    assert.match(home.text, /지금 실행할 카드/);
    assert.match(home.text, /flow-chain-spotlight-title/);
    assert.match(home.text, /카드를 클릭하면 상세 설명과 수정 맵이 아래에 열립니다\./);
    ...
    assert.match(home.text, /실행 화면에 넣기/);
    assert.match(home.text, /source=home-spotlight/);
    assert.match(home.text, /바로 열기/);
    assert.match(home.text, /클릭해서 수정 맵 보기/);
    assert.match(home.text, /flow-chain-map-panel/);
    assert.match(home.text, /이 단계 설명과 해결 순서/);
    assert.match(home.text, /바로 보면 좋은 기준/);
    assert.match(home.text, /bindHomeOperatorChainInteractions/);
    assert.match(home.text, /homeOperatorChainMapHtml/);
