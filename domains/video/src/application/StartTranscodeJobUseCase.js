 ...
 * NFR: CPU-bound 트랜스코딩은 workerAdapter.dispatch()로 Worker Thread 분리 실행
 */
class StartTranscodeJobUseCase {
   ...
   *   workerAdapter?: { dispatch: (job: object) => void },
   * }} deps
   */
  constructor({ videoRepository, transcodeJobRepository, workerAdapter = null }) {
    this._videoRepo    = videoRepository;
    this._jobRepo      = transcodeJobRepository;
    this._workerAdapter = workerAdapter;
  }
  ...
  async execute(cmd, caller) {
    ...
    const savedJob = await this._jobRepo.save(job);

    // NFR: CPU-bound 트랜스코딩을 Worker Thread로 fire-and-forget 분리
    // 메인 이벤트 루프 블로킹 없이 즉시 PENDING job 반환
    if (this._workerAdapter) {
      this._workerAdapter.dispatch({
        jobId:            savedJob.jobId,
        videoId:          savedJob.videoId,
        targetFormat:     savedJob.targetFormat,
        targetResolution: savedJob.targetResolution,
      });
    }
